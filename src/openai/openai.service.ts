  ...
  private buildSystemPrompt(
    ...
    if (memoryContext?.userDefaults && Object.keys(memoryContext.userDefaults).length > 0) {
      prompt +=
        ...
        'ВАЖНО: если пользователь НЕ указал город вылета, но в дефолтах есть departureCity — ' +
        'ПОДСТАВЬ ЕГО В parsed.departureCity и НЕ СПРАШИВАЙ. Это уже известный город вылета пользователя.\n' +
