...
export class OpenAiService {
  ...
  private buildSystemPrompt(
    ...
    let prompt =
      ...
      '- НИКОГДА не подставляй параметры сам. Если чего-то не знаешь — СПРОСИ. Единственное исключение: если параметр уже известен из памяти (факты, прошлые поиски) — уточни: "В прошлый раз летели из Москвы — снова оттуда?"\n\n' +
    ...
    if (memoryContext?.userDefaults && Object.keys(memoryContext.userDefaults).length > 0) {
      prompt +=
        ...
        'Используй как подсказку для уточняющих вопросов. НЕ подставляй молча — УТОЧНИ: "В прошлый раз искали из X в Y — что-то похожее?"\n\n';
    }
